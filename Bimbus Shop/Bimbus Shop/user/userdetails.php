<?php
	...
	if (@$_SESSION['user']) {
		...
		$sql = mysqli_query($connection, "SELECT * FROM user WHERE username = '$user'") or die(mysql_error());
		$data = mysqli_fetch_array($sql);
		$result = mysqli_num_rows($sql);
		$usertype = $data['access'];
		if ($usertype === "user") {
			$userid = $user;
			$sql = mysqli_query($connection, "SELECT * FROM user WHERE username = '$userid'") or die(mysql_error());
			$data = mysqli_fetch_array($sql);
			$result = mysqli_num_rows($sql);
...
<body>
	...
	<ul id="navitop" class ="navitop">
		...
		<li class="list"><a href="homepage.php">Bimbus</a></li>
